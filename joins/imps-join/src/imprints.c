...
simd_imprints(Column *column, Imprints_index *imps)
{
	...
	__m256i       *restrict limits = (__m256i *) malloc_aligned((imps->bins) * sizeof(__m256i));
...
/**
 * add one more parameter compared to create_imprints(...)
 *
 * @param impsidxToBeAligned: if not NULL, create imprints
 * according to impsidxToBeAligned's bin number and bounds
 */
Imprints_index*
create_imprints_aligned(Column *column,
		int blocksize,
		int max_bins,
		int simd,
		Imprints_index * impsidxToBeAligned)
{
	Imprints_index *imps = column->imps_idx;
	...
	imps->bounds      = (ValRecord *) malloc_aligned(sizeof(ValRecord) * max_bins);
	max_imprints      = (column->colcount/(imps->blocksize/column->typesize))+1;
	imps->dct         = (Dct *) malloc_aligned(sizeof(Dct)  * max_imprints);
	imps->dct_cnt     = 0;
	imps->imps_cnt    = 0;

	/* no need to decide the bounds at this time */
...
binning(Column *column, ValRecord *bounds, int *bins, int max_bins) {
	...
	sample = (ValRecord *) malloc_aligned(sizeof(ValRecord)*SAMPLE_SZ);
...
int buildImpsR2L(Column *colL, Column *colR, int max_bins, int blocksize) {
	...
	Imprints_index * colR_impsidx = colR->imps_idx;

	/* decides the bin borders for column R */
	colR_impsidx->bounds = (ValRecord *) malloc_aligned(sizeof(ValRecord) * max_bins);
...
int buildImpsL2R(Column *colL, Column *colR, int max_bins, int blocksize) {
	...
	Imprints_index * colL_impsidx = colL->imps_idx;

	/* decides the bin borders for column R */
	colL_impsidx->bounds = (ValRecord *) malloc_aligned(sizeof(ValRecord) * max_bins);

	binning(colR, colL_impsidx->bounds, &(colL_impsidx->bins), max_bins);

	/* create imprints for column L according to colR_impsidx->bounds */
	create_imprints_aligned(colL, blocksize, max_bins, 1, colL_impsidx);
